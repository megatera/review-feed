...
module.exports = async function generateDailyDigest (appId, limit) {
  // fetch reviews
  const raw = await axios.get(`https://itunes.apple.com/us/rss/customerreviews/id=${appId}/sortBy=mostRecent/page=1/json`);
  ...
  let mostRecent = null;
  const regEx = /^### Date: (.*$)/m        
  // 'a+' flag to create cache file if it doesn't exist
  const cacheData = fs.readFileSync(`cache_${appId}.md`, { flag: 'a+' }).toString();
  ...
  if (mostRecent !== null) {
    reviews = reviews.filter((entry) => new Date(entry.updated.label) > mostRecent);
  }

  // iterate over fetched reviews up (till desired limit) and convert to markdown
  let dailyDigest = '';

  for (let i = reviews.length - 1; i > reviews.length - 1 - limit && i >= 0; i--) { //new Date(reviews[i].updated) > mostRecent
  ...
  // generate daily digest and update cache file
  let timestamp = Date(); //TODO: formatting
